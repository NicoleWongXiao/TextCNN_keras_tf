...
def train_step(x_batch, y_batch,train_op):
    ...
    _, step, summaries, loss, accuracy = sess.run(
        ...
        feed_dict)
    time_str = datetime.datetime.now().isoformat(sep=' ')
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        cnn = TextCNN(
            sequence_length=x_train.shape[1],             # 填充后的所有句子的长度（我们的数据集为59）
            num_classes=y_train.shape[1],                 # 分类数：两个（正数和负数）。
            vocab_size=len(vocab_processor.vocabulary_),  # 邮件字典：不重复单词数目
            embedding_size=FLAGS.embedding_dim,           # 词嵌入层的维度
        ...
        # var_list: 需要更新的tf.Varialble组成的列表或者元组
        # 若var_list中的变量个数少于var(loss)，则优化过程中只会更新var_list中的那些变量的值，
        # var(loss)里多出的变量值 并不会改变，相当于固定了网络的某一部分的参数值。
        # 如果var_list里所包含的变量多于var(loss)，则程序会报错。

        # compute_gradients的返回值：(gradient, variable)对 所组成的列表，返回的数据格式也都是“tf.Tensor”。
        # 可以通过变量名称的管理来过滤出里面的部分变量，以及对应的梯度。
        grads_and_vars = optimizer.compute_gradients(cnn.loss)

        # apply_gradients 更新grads_and_vars中variable的梯度，不在里面的变量的梯度不变。
        ...
        for g, v in grads_and_vars:
            if g is not None:
                # tf.summary.histogram：用来显示直方图信息 ；
                # 显示训练过程中变量grads_and_vars的分布情况
        ...
        # Train Summaries ：summary.merge 将所有的 summary 全部保存到磁盘
